...
class F139GdFx(F139Price):
    job_name = '富宝报价抓取——广东市场废锡价格行情'
    data_url = "http://data.f139.com/list.do?pid=&vid=20&qw=3:27"
    title = '{}{}'.format(f139_config.prefix_of_title, "广东市场废锡价格行情")
    ...
    def run(self):
        ...
        # 第五列：价格
        fifth_column = ['不含税价（元/公斤）']
        fifth_column2 = [each_row.xpath('text()')[0].strip() for each_row in price[1:]]
        fifth_column.extend(['{}-{}'.format(str(int(each_price.split('-')[0])),
                                            str(int(each_price.split('-')[-1])))
            ...
            if text_flat and text_flat != ['\r\n\t\t\t\t\t\t\t', '\r\n\t\t\t\t\t']:
                ...
                if text_rise:
                    ...
                    if text_fall:
                        ...
                        sixth_column.append('')
        # 整合表格
        first_column = first_column[0:3] + first_column[5:7]
        second_column = second_column[0:3] + second_column[5:7]
        fourth_column = fourth_column[0:3] + fourth_column[5:7]
        fifth_column = fifth_column[0:3] + fifth_column[5:7]
        sixth_column = sixth_column[0:3] + sixth_column[5:7]
        table = zip(first_column, second_column, third_column, fourth_column, fifth_column, sixth_column)
        single_tr = []
