...
class F139ShFl(F139Price):
    job_name = '富宝报价抓取——上海地区废铝价格'
    data_url = "http://data.f139.com/list.do?pid=&vid=17&qw=3:258"
    title = '{}{}'.format(f139_config.prefix_of_title, "上海地区废铝价格")
    ...
    def run(self):
        ...
        # 第三列：品名
        third_column = ['品名']
        third_column.extend([each_row.xpath('text()')[0].strip().replace('易拉罐压块', '易拉罐') for each_row in name])
        # 第四列：价格
            ...
            if text_flat and text_flat != ['\r\n\t\t\t\t\t\t\t', '\r\n\t\t\t\t\t']:
                ...
                if text_rise:
                    ...
                    if text_fall:
                        ...
                        fifth_column.append('')
        # 整合表格
        table = zip(first_column[0:6], second_column[0:6], third_column[0:6], fourth_column[0:6], fifth_column[0:6])
        single_tr = []
