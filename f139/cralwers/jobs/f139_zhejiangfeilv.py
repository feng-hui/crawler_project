...
__author__ = 'Feng_hui'
__time__ = '2018/2/28 9:23'


class F139ZjFl(F139Price):
    job_name = '富宝报价抓取——浙江地区废铝价格'
    data_url = "http://data.f139.com/list.do?pid=&vid=17&qw=3:50"
    title = '{}{}'.format(f139_config.prefix_of_title, "浙江地区废铝价格")
    ...
    def run(self):
        ...
        # 第一列：地区
        first_column = [each_row.xpath('text()')[0].strip().replace('地区/来源', '省份').replace('台州', '浙江')
            ...
            if text_flat and text_flat != ['\r\n\t\t\t\t\t\t\t', '\r\n\t\t\t\t\t']:
                ...
                if text_rise:
                    ...
                    if text_fall:
                        ...
                        fifth_column.append('')
        # 整合表格
        first_column = first_column[0:2] + first_column[6:10] + [first_column[11]]
        second_column = second_column[0:2] + second_column[6:10] + [second_column[11]]
        third_column = third_column[0:2] + third_column[6:10] + [third_column[11]]
        fourth_column = fourth_column[0:2] + fourth_column[6:10] + [fourth_column[11]]
        fifth_column = fifth_column[0:2] + fifth_column[6:10] + [fifth_column[11]]
