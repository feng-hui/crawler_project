...
class AHospitalSpider(scrapy.Spider):
    ...
    # start_urls = ['http://www.a-hospital.com/w/%E7%BB%B5%E9%98%B3%E5%B8%82%E5%8C%BB%E9%99%A2%E5%88%97%E8%A1%A8']
    ...
    test_area_url = 'http://www.a-hospital.com/w/%E5%93%88%E5%B0%94%E6%BB%A8%' \
                    'E5%B8%82%E5%8C%BB%E9%99%A2%E5%88%97%E8%A1%A8'
    headers = {
    ...
    def start_requests(self):
        ...
        # 测试地区页面
        # self.headers['Referer'] = 'http://www.a-hospital.com/w/%E5%85%A8%E5%9B%BD%E5%8C%BB%E9%99%A2%E5%88%97%E8%A1%A8'
        # yield Request(self.test_area_url, headers=self.headers, callback=self.parse_area, dont_filter=True)
    ...
    def parse_area(self, response):
        ...
        try:
            ...
            for each_special_hospital in special_hospital_list:
                ...
                hospital_address = each_special_hospital.xpath('ul[1]/li/b[contains(text(),'
                                                               '"医院地址")]/ancestor::li[1]/text()').extract_first('')
                hos_county = hos_county if hos_county else get_county(hos_prov, hos_city, hospital_address)
                loader = CommonLoader2(item=HospitalInfoTestItem(), selector=each_special_hospital)
                ...
                # 科室信息
                dept_info = each_special_hospital.xpath('ul[1]/li/b[contains(text(),"重点科室")]/ancestor::li[1]')
                ...
                if all_dept_info:
                    for each_dept in all_dept_info.split('、'):
                        ...
                        dept_loader.add_value('dept_name', each_dept, MapCompose(custom_remove_tags))
                        dept_loader.add_value('hospital_name',
                                              hospital_name,
                ...
                # 医院别名信息
                hospital_name2 = each_special_hospital.xpath('b/text()').extract_first('')
                if hospital_name2 and '（' in hospital_name2:
                    alias_name = re.search(r'（(.*?)）', custom_remove_tags(hospital_name2))
                    if alias_name:
                        for each_alias_name in alias_name.group(1).split('、'):
                            alias_loader = CommonLoader2(item=HospitalAliasItem(), response=response)
                            alias_loader.add_value('hospital_name',
                                                   hospital_name,
                                                   MapCompose(custom_remove_tags, match_special2))
    ...
    def parse_hospital_detail(self, response):
        ...
        # 医院信息item
        hospital_name1 = response.xpath('//table[@class="nav"]/tr/td/strong/text()').extract_first('')
        ...
        loader.add_value('hospital_name',
                         hospital_name,
                         MapCompose(custom_remove_tags, match_special2))
        loader.add_xpath('hospital_level',
        ...
        if all_dept_info:
            for each_dept in all_dept_info.split('、'):
                dept_loader = CommonLoader2(item=HospitalDepItem(), response=response)
                dept_loader.add_value('dept_name', each_dept, MapCompose(custom_remove_tags))
                dept_loader.add_value('hospital_name', hospital_name, MapCompose(custom_remove_tags, match_special2))
                dept_loader.add_value('update_time', now_day())
                dept_item = dept_loader.load_item()
                yield dept_item
